...
```
; pip install git+https://github.com/uwuzone/enumerate-iam
; enumerate-file                         # original script
; enumerate-iam-file < keys.json         # test keys from stdin
; enumerate-iam-file --json < keys.json  # print the results as json
```

When testing from stdin, you must provide keys in this format with _one key per line_ (`jq -c` might help)
...
The `.source` key is for your documentation.

```json
{
  "access_key_id": "AKIA...",
  "secret_access_key": "...",
  "source": "scan:travis-ci:example:repo"
}
```

Control log levels by setting `ENUMERATE_IAM_LOG_LEVEL` in the env. Valid values are `DEBUG, INFO, WARNING, ERROR`.
